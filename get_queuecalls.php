...
if ($debug) { echo '<html><body><table><thead><tr><th>debug activated</th></tr></thead><tbody>'; } 
	...
	if(apcu_exists("queuecall_".$site."_".$queue)) {
		echo apcu_fetch("queuecall_".$site."_".$queue);
		exit;
	}
}
...
$resp = json_encode(simplexml_load_string(get_http('http://'.urlencode($site).'/xml/ccGetQueueStatus.php?queue='.urlencode($queue))));

if ($debug) { echo '</tbody></table></body></html>'; }
else { apcu_store("queuecall_".$site."_".$queue, $resp, 1); } 
echo $resp;
?>